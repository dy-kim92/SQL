...
--  묵시적 방법 : 데이터를 넣은 컬럼을 지정하지 않는 경우
--   정의된 순서에 따라서 값을 입력
INSERT INTO author
VALUES(1, '박경리', '토지 작가');

--  명시적 방법 
--  데이터를 넣을 컬럼을 지정, 지정한 컬럼 순서대로 데이터 제공,
--  명시되지 않은 컬럼은 NULL 입력 -> NOT NULL 제약조건이 있는 필드는 오류 발생
...
--  INSERT, UPDATE, DELETE 작업을 수행하면 Transaction 이 수행됨
--  작업이 완료되었을 때
--  원래대로 복원 : ROLLBACK
--  영구 저장 : COMMIT
COMMIT;
...
--  author_id는 author.author_id의 FK를 참조하고 있으므로 제약조건 위반
...
--  UPDATE 시 조건을 부여하지 않으면 모든 레코드 변경 -> 주의
ROLLBACK;
SELECT * FROM author;
...
--  주의 : TRUNCATE는 Transaction의 대상이 아니다 -> ROLLBACK 불가능
ROLLBACK;
SELECT * FROM emp_123;
