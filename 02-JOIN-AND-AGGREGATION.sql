...
SELECT COUNT(*) FROM employees;     --  * 는 전체 레코드 카운트를 집계 (내부 값이 null 이 있어도 집계)
SELECT COUNT(commission_pct) FROM employees;    --  특정 컬럼을 명시하면 null 인 것은 집계에서 제외
...
SELECT AVG(nvl(commission_pct, 0)) FROM employees;  --  null 데이터 포함

--  null 이 포함된 집계는 null 값을 포함할 것인지 아닌지를 결정하고 집계

--  salary 의 최소값, 최대값, 평균값 중앙값
...
--  department_id -> 여러 행, AVG(salary) -> 단일 행

--  부서별 평균 연봉을 구하려면 ?
--  부서별 Group 을 지어준 데이터를 대상으로 집계 함수 수행
...
--  부서별 평균 급여를 산출 : 평균 급여가 2000 이상인 부서를 출력
SELECT department_id, AVG(salary)
FROM employees
WHERE AVG(salary) >= 20000  --  이 시점에는 AVG(salary)가 수행되지 않은 상태 : 없음
GROUP BY department_id;
--  ERROR : 집계 작업이 일어나기 전에 WHERE 절이 실행됨
...
SELECT salary, first_name,
    ...
    DENSE_RANK() OVER (ORDER BY salary DESC) as densd_rank, --  중복순위 상관 없이 다음 순위 부여
...
--  employees 테이블로 조직도를 그려보자
--  level(깊이)라는 가상 컬럼을 사용할 수 있다
